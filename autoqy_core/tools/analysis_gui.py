...
(() => {
  ...
  document.addEventListener('click', (event) => {
        ...
        html.Main(className="workspace analysis-workspace", children=[
            html.Aside(className="control-column analysis-controls", children=[
                ...
                html.Details(open=False, className="panel tool-details", children=[
                    ...
                    html.Label("Method"),
                    dropdown("fit_method", (
                        ("nipe", "NIPE (checks for degradation)"),
                    ...
                    html.Details(className="nested-tool", children=[
                        html.Summary(["Method-specific controls", info_popup(
                            "NIPE compares changes between spectra and checks whether the sample still behaves "
                            "like a simple two-species reaction. The other controls below apply only to the "
                            "method named in their label; the defaults are suitable for most analyses."
                        )]),
            ...
            html.Div(className="plot-column analysis-results", children=[
                html.Section(className="result-strip analysis-result-strip", children=[
                    ...
                    html.Div([
                        html.Span("Extrapolated PSS"),
                        html.Div([
                            html.Div([html.Span("Reactant"), html.Strong("—")],
                                     className="pss-species"),
                            html.Div([html.Span("Product"), html.Strong("—")],
                                     className="pss-species"),
                        ], className="pss-distribution"),
                        html.Small("Photostationary-state distribution"),
                    ], id="result-pss", className="result-card result-card-pss"),
                    html.Div([html.Span("Fit"), html.Strong("—"), html.Small("Not run")],
                             id="result-fit", className="result-card result-card-neutral"),
                ]),
                html.Div(id="nipe-headline-warning", className="nipe-headline-warning"),
                html.Details(id="diagnostic-panel", open=False,
                             className="panel diagnostic-panel tool-details", children=[
                    html.Summary([
                        "Preprocessing and fit diagnostics",
                        info_popup(
                        "AutoQY checks the input files and the fitted model. Green means the check passed, "
                        "amber means the result should be reviewed, and red means the result should not be "
                        "treated as a normal two-species fit. Open the information icon beside each check "
                        "for its specific explanation."
                        ),
                    ]),
                ...
                html.Details(id="nipe-window-panel", open=False,
                    ...
                    html.Summary([
                        ...
                        info_popup(
                            "AutoQY compares short sections before the concentration curve becomes flat. "
                            "If the fitted yield changes with time, it estimates the yield near the start, "
                            "before degradation has a large effect."
                        ),
                    ]),
                    html.Div(
                ...
                html.Section(className="plot-panel analysis-plot-panel", children=[
                    dcc.Tabs(id="analysis-plot-tabs", value="spectra", children=[
                        dcc.Tab(label="Concentrations", value="concentrations",
                                children=[
                                    html.Div(id="nipe-concentration-note"),
                                    analysis_graph("concentration-figure"),
                                ]),
    ...
    @app.callback(
        ...
        Output("result-pss", "children"),
        Output("result-fit", "children"),
        Output("nipe-headline-warning", "children"),
        Output("nipe-concentration-note", "children"),
    ...
    def analyze(_, __, ___, current_values, field_ids):
        ...
        blank_pss = _pss_card(html)
        blank_fit = [html.Span("Fit"), html.Strong("—"), html.Small("Not run")]
        blank_headline_warning = ""
        blank_concentration_note = ""
        blank_figures = [
        ...
        try:
            ...
            diagnostics_open = (True if any(
                item["level"] == "stop" for item in input_checks
            ) else no_update)
            ...
            if action == "save-analysis-json":
                ...
                if not selected:
                    return ("Save cancelled.", "message", blank_card, blank_back, blank_pss,
                            blank_fit, blank_headline_warning,
                            blank_concentration_note,
                ...
                return (f"Saved and validated {target}", "message status-message status-ok",
                        ...
                        blank_headline_warning, blank_concentration_note, *blank_figures,
            ...
            headline_warning = _nipe_headline_warning(html, summary)
            concentration_note = _nipe_concentration_note(
                html, output.result, output.data,
            )
            ...
            return (message, message_class,
                    rp, pr, pss, fit, headline_warning, concentration_note,
                    *figures, diagnostics,
            ...
            return ("Analysis stopped. See Python errors.",
                    "message status-message status-stop", blank_card, blank_back, blank_pss,
                    blank_fit, blank_headline_warning,
                    blank_concentration_note,
                    *blank_figures,
...
def _spectra_led_figure(go, make_subplots, wavelengths, absorbance,
    ...
    visible_epsilon_max = 0.0
    for reference, name, colour in (
        (epsilon_r, "Reactant ε", PLOT_BLUE),
        (epsilon_p, "Product ε", PLOT_ORANGE),
    ):
        reference_wavelengths = np.asarray(reference[0], float)
        reference_values = np.asarray(reference[1], float)
        reference_mask = ((reference_wavelengths >= low)
                          & (reference_wavelengths <= high))
        if np.any(reference_mask):
            reference_wavelengths = reference_wavelengths[reference_mask]
            reference_values = reference_values[reference_mask]
        finite_values = reference_values[np.isfinite(reference_values)]
        if len(finite_values):
            visible_epsilon_max = max(
                visible_epsilon_max, float(np.max(finite_values)),
            )
        figure.add_trace(go.Scatter(
            x=reference_wavelengths, y=reference_values,
    ...
    led_mask = (led_wavelengths >= low) & (led_wavelengths <= high)
    if np.any(led_mask):
        led_wavelengths = led_wavelengths[led_mask]
        led_values = led_values[led_mask]
    led_scale = max(float(np.max(led_values)), np.finfo(float).eps)
    normalized_led = led_values / led_scale
    figure.add_trace(go.Scatter(
        x=led_wavelengths, y=normalized_led, mode="lines",
    ...
    epsilon_upper = 1.05 * visible_epsilon_max if visible_epsilon_max > 0 else 1.0
    finite_led = normalized_led[np.isfinite(normalized_led)]
    led_upper = max(float(np.max(finite_led)), 1.0) * 1.05 if len(finite_led) else 1.05
    figure.update_yaxes(title_text="ε (M⁻¹ cm⁻¹)", range=[0, epsilon_upper],
                        autorange=False, row=1, col=1, secondary_y=False)
    figure.update_yaxes(title_text="LED (normalized)", range=[0, led_upper],
                        autorange=False, showgrid=False, row=1, col=1,
                        secondary_y=True)
...
def _nipe_headline_warning(html, summary):
...
def _nipe_fit_display_window(result, timestamps):
    """Return the samples where a NIPE fit should be displayed and its end time."""
    times = np.asarray(timestamps, dtype=float)
    full_mask = np.ones(times.shape, dtype=bool)
    if result.fit_method != "nipe" or times.size == 0:
        return full_mask, None
    nipe = getattr(result.yield_fit, "nipe", None)
    window = getattr(nipe, "window_analysis", None)
    ...
    if not np.any(mask):
        return full_mask, None
    return mask, float(times[mask][-1])


def _nipe_concentration_note(html, result, data):
    if result.fit_method != "nipe":
        return ""
    _, end_time = _nipe_fit_display_window(result, data.timestamps)
    if end_time is None:
        message = (
            "No reliable early-time cutoff was found, so the full fitted trace is shown. "
            "Measured A + B is allowed to change during a NIPE fit."
        )
    else:
        message = (
            f"Measured points are shown for the complete experiment. The fitted lines and "
            f"ε ranges stop at {end_time:.3g} s, the end of the accepted early-time "
            "window; points after the dashed line are not fitted."
        )
    return html.Div([
        html.Strong("About this NIPE plot: "), message,
    ], className="message status-message nipe-concentration-note")
...
def _render_nipe_window_analysis(html, summary, reactant_name, product_name):
    ...
    for item in analysis["windows"]:
        window_rp = format_value_uncertainty(
            item["R_to_P_percent"], item["R_to_P_standard_error_percent"],
            two_digit_threshold=2,
        )
        window_pr = format_value_uncertainty(
            item["P_to_R_percent"], item["P_to_R_standard_error_percent"],
            two_digit_threshold=2,
        )
        rows.append(html.Tr([
            html.Td(f"{item['start_s']:.3g}–{item['end_s']:.3g}"),
            html.Td(f"{window_rp[0]} ± {window_rp[1]}"),
            html.Td(f"{window_pr[0]} ± {window_pr[1]}"),
    ...
    table = html.Div(className="nipe-window-table-wrap", children=[
        html.Table(className="nipe-window-table", children=[
            html.Thead(html.Tr([
                ...
                html.Th([
                    "Condition",
                    html.Span(className="info-popup nipe-condition-info", tabIndex=0,
                              children=[
                                  html.Span("i", className="info-popup-icon",
                                            **{"aria-hidden": "true"}),
                                  html.Span(
                                      "Shows how reliably the two directional quantum yields "
                                      "can be separated in this time window. Lower values are "
                                      "better. Very large values mean that small measurement "
                                      "errors can strongly change the fitted yields; windows "
                                      "above 10,000 are rejected automatically.",
...
def _pss_card(html, summary=None, reactant_name="Reactant", product_name="Product"):
    values = (summary or {}).get("extrapolated_pss_percent", {})
    reactant = values.get("reactant")
    ...
    return [
        html.Span("Extrapolated PSS"),
        html.Div([
            html.Div([html.Span(reactant_name), html.Strong(displayed(reactant))],
                     className="pss-species"),
            html.Div([html.Span(product_name), html.Strong(displayed(product))],
                     className="pss-species"),
        ], className="pss-distribution"),
        html.Small("Photostationary-state distribution"),
    ]
...
def _render_checks(html, checks):
    labels = {"ok": "OK", "warning": "Review", "stop": "Red flag"}
    return [html.Div(className=f"diagnostic-item diagnostic-{item['level']}", children=[
        html.Div(className="diagnostic-title-row", children=[
            html.Strong(item["title"]),
            html.Span(className="info-popup", tabIndex=0, children=[
                html.Span("i", className="info-popup-icon", **{"aria-hidden": "true"}),
                html.Span(item["body"], className="info-popup-content"),
            ]),
        ]),
        html.Span(labels[item["level"]], className="diagnostic-level"),
    ]) for item in checks]
...
def _interactive_figures(go, make_subplots, result, data, residual_percentile,
    ...
    if uncertainty is not None:
        for index, (name, colour) in enumerate((("Reactant", blue), ("Product", orange))):
            _add_band(go, concentration, fit_times,
                      uncertainty.concentration_fit_minimum[fit_mask, index],
                      uncertainty.concentration_fit_maximum[fit_mask, index], colour,
    ...
    for index, (name, colour) in enumerate((("Reactant", blue), ("Product", orange))):
        concentration.add_trace(go.Scatter(
            x=fit_times, y=fitted[fit_mask, index], mode="lines", name=f"{name} fit",
    ...
    if fit_end_time is not None:
        concentration.add_vline(
            x=fit_end_time,
            line={"color": PLOT_BROWN, "dash": "dash", "width": 2},
            annotation_text="NIPE fit window ends",
            annotation_position="top right",
            annotation_font={"color": PLOT_BROWN, "size": 11},
        )
...
def _fit_note(summary):
    nipe = summary.get("nipe")
    if nipe:
        return "See NIPE pre-plateau window analysis"
