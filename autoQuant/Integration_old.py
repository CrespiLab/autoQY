...
from .Constants import h, c, Avogadro
from .ExpParam import V, k_BA, I0_avg, I0_err
...
def Import_SpectralData(FileFormat, file, 
                        wl_low, wl_high, wavelength_of_interest):
    ...
    if FileFormat == "Spectragryph":
        ...
        data_pd = pd.read_csv(file, delimiter=' ' , header=None)# sep = ' ', header=None) # in nanometers ####ALFREDO: FIXED?
...
'''
def Import_SpectralData(FileFormat, file, 
                        wl_low=None, wl_high=None, wavelength_of_interest=None):
    """Import spectral data and return filtered data based on wavelength range."""

    if FileFormat == "Spectragryph":
        data_pd = pd.read_csv(file, sep='\t', usecols=lambda x: x not in ["Wavenumbers [1/cm]"])
    elif FileFormat == "Not":
        data_pd = pd.read_csv(file, delimiter=',', header=None)

    # Ensure the first column is numeric (wavelengths)
    data_pd.iloc[:, 0] = pd.to_numeric(data_pd.iloc[:, 0], errors='coerce')
    
    # Drop any rows where the first column has invalid (NaN) values
    data_pd.dropna(subset=[data_pd.columns[0]], inplace=True)
    ...
    # Handle missing wl_low and wl_high by setting them to min/max of the data
    #if wl_low is None:
    #    wl_low = np.min(data[:, 0])
    #if wl_high is None:
    #    wl_high = np.max(data[:, 0])

    # Find indices corresponding to the low and high wavelength range
    low = np.argmin(np.abs(data[:, 0] - wl_low))
    high = np.argmin(np.abs(data[:, 0] - wl_high))

    # Select wavelengths and absorbance within the range
    wavelengths_lowhigh = data[low:high, 0]
    absorbance_lowhigh = data[low:high, 1:]
    print(wavelengths_lowhigh)
    print(absorbance_lowhigh)
    # Find index closest to the wavelength of interest
    index = np.abs(wavelengths_lowhigh - wavelength_of_interest).argmin()
...
##################################################
'''
def Plot_LEDemission_Processed(absorbance, wavelengths,
                               em_wl, em_int,
...
def Interpolate_Epsilons(Abs_wavelengths,
    ...
    ## wavelengths data
    ##########################################################################
    # Convert all inputs to float (if not already done)
    Abs_wavelengths = pd.to_numeric(Abs_wavelengths, errors='coerce')
    eA_wavelengths = pd.to_numeric(eA_wavelengths, errors='coerce')
    eA_values = pd.to_numeric(eA_values, errors='coerce')
    eB_wavelengths = pd.to_numeric(eB_wavelengths, errors='coerce')
    eB_values = pd.to_numeric(eB_values, errors='coerce')
    LED_wavelengths = pd.to_numeric(LED_wavelengths, errors='coerce')
    LED_intensity_proc = pd.to_numeric(LED_intensity_proc, errors='coerce')

    # Drop any NaN values that might result from conversion errors
    if any(pd.isnull(Abs_wavelengths)) or any(pd.isnull(eA_wavelengths)) or any(pd.isnull(eB_wavelengths)) or any(pd.isnull(LED_wavelengths)):
        raise ValueError("Invalid data: one or more wavelength arrays contain NaN values.")

    # Ensure that wavelengths and intensities are numeric arrays
    Abs_wavelengths = np.array(Abs_wavelengths, dtype=np.float64)
    eA_wavelengths = np.array(eA_wavelengths, dtype=np.float64)
    eA_values = np.array(eA_values, dtype=np.float64)
    eB_wavelengths = np.array(eB_wavelengths, dtype=np.float64)
    eB_values = np.array(eB_values, dtype=np.float64)
    LED_wavelengths = np.array(LED_wavelengths, dtype=np.float64)
    LED_intensity_proc = np.array(LED_intensity_proc, dtype=np.float64)
