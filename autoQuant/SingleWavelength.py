...
V = 2.0 ## Volume in ml
k_BA = 0 ## Thermal back reaction rate s-1
####################################################
...
index=find_closest_index(data, wavelength_nm)
absorbance_values = data[index, 1:]

# Initial concentration of A (based on the first experimental value)
initial_conc_A = absorbance_values[0] / epsilon_A
initial_conc_B = 0.0
initial_conc = initial_conc_A, initial_conc_B

# fig, axdata = plt.subplots(1, 2, figsize=(10, 4),dpi=600,constrained_layout=True)
fig = plt.figure(figsize=(8, 4),dpi=600,constrained_layout=True)
gs = gridspec.GridSpec(1, 6, figure=fig)

axdata1 = fig.add_subplot(gs[0, 0:4])
axdata2 = fig.add_subplot(gs[0, 4:])


axdata1.set_title("Data")
axdata1.set_xlabel("Wavelength (nm)")
axdata1.set_ylabel("Absorbance")
for i in range(1,len(data.T)):
    axdata1.plot(data.T[0],data.T[i])

axdata2.set_title(f"Absorbance at {wavelength_nm} nm")
axdata2.set_xlabel("Spectrum")
axdata2.plot(absorbance_values)

plt.show()

#%% CALCULATE QYs, PLOT AND SAVE
## Defining the functions 
def rate_equations(concentrations, time, QY_AB, QY_BA):
    """
    ...
    """
    A, B = concentrations
    total_absorbance = A * epsilon_A + B * epsilon_B

    dAdt = (N/V) * ((1 - 10 ** (-total_absorbance ))/total_absorbance) \
    * (QY_BA * B * epsilon_B - QY_AB * A * epsilon_A) + k_BA * B
